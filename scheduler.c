...
void job_submission_thread(){
    int*  thread = (int*)arg;
    struct timespec start, stop;
    while(1){
        job* j = malloc(sizeof(job));
        free(j->phase);// to free unused malloc
...
int main() {
    ...
    for(int i = 0 ; i <= 4; i++){
        ...
        temp = &i;
        pthread_create(&job_submission[i], NULL,job_submission_thread,(void *) temp);
    }
